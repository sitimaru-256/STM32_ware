...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(motorState == 1){
	  		if(basfrq >= 60 && dir == 1){pulse_mode = CHM;alpha_num = 0;}
	  		else if(basfrq >= 43){pulse_mode = S3p;}
	  		else if(basfrq >= 36){pulse_mode = SAMI;alpha_num = 2;}
	        else if(basfrq >= 28){pulse_mode = SyncMp;pnum_PWM = 9;}
	  		else if(basfrq >= 0){pulse_mode = Async;frq = 385;}
	  	}

	  	else if(motorState == -1){
	  		if(basfrq >= 72 && dir == -1){pulse_mode = CHM;alpha_num = 0;}
	  		else if(basfrq >= 43){pulse_mode = S3p;}
	  		else if(basfrq >= 36){pulse_mode = SAMI;alpha_num = 2;}
	  		else if(basfrq >= 24){pulse_mode = SyncMp;pnum_PWM = 9;}
	  		else if(basfrq >= 0){pulse_mode = Async;frq = 385;}
	  	}
...
void modulation_acc (float a){
	ratio = min(max(a * 8.775862, basfrq * 3.3933335), 502);
