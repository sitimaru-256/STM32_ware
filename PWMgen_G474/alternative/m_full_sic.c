...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    ...
    if(htim == &htim7){
    	basfrq += (0.0005 * dir);
    	if(JerkPole != 0){basfrq_Jerk += (0.004*JerkPole);}
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if(motorState == 1){
		  if(basfrq >= 66.5 && dir == 1){pulse_mode = SyncMp;pnum_PWM = 27;}
		  else if(basfrq >= 66.5){pulse_mode = SyncMp;pnum_PWM = 15;}
		  else if(basfrq >= 0){pulse_mode = Async;frq = 1100;async_mode = RPPWM;randnum = 200;}
	  	}
	  else if(motorState == -1){
		  if(basfrq >= 80 && dir == -1){pulse_mode = SyncMp;pnum_PWM = 27;}
		  else if(basfrq >= 80){pulse_mode = SyncMp;pnum_PWM = 15;}
		  else if(basfrq >= 0){pulse_mode = Async;frq = 1100;async_mode = RPPWM;randnum = 200;}
	  	}
...
void modulation_acc (float a){
	if(basfrq >= 66.5){
		AMP = min(max(0.072593*a - 3.807407, 0.03125*basfrq - 2), 2);
		OFFSET = min(max(-0.036296*a + 2.403704, -0.5), -0.015625*basfrq + 1.5);
	}
	else if(basfrq >= 0){
		AMP = min(max(a * 0.015338, basfrq*0.00625), 1.02);
		OFFSET = min(max(0.5 - a * 0.007669, -0.01), 0.5 - basfrq * 0.003125);
	}
	ratio = min(max(44.74*a - 1365.1597, 1), 502);
}
void modulation_dec (float a){
	if(basfrq >= 80){
		AMP = min(max(0.0625*a - 4, 0.03125*basfrq - 2), 2);
		OFFSET = min(max(-0.03125*a + 2.5, -0.5), -0.015625*basfrq + 1.5);
	}
	else if(basfrq >= 0){
		AMP = min(max(a * 0.0125, basfrq*0.00625), 1);
		OFFSET = min(max(0.5 - a * 0.00625, 0), 0.5 - basfrq * 0.003125);
	}
